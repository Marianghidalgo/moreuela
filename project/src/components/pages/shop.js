...
class Shop extends Component {
  ...
  removeCartArticles(event, item) {
    const filterCartArticles = this.state.cartArticles.filter(
  ...
  getShopArticles() {
    axios
      ...
      .catch((error) => {
        console.error("getShopArticles error", error);
      });
  }
    ...
    const shopping = this.state.shopArticles.map((shopArticle) => {
      return (
        <ShopArticle
          ...
          addCartArticles={this.setCartArticles}
        />
      );
    });
    ...
    const cart = this.state.cartArticles.map((cartArticle) => {
      return (
        <CartArticle
          ...
          removeCartArticles={this.removeCartArticles}
        />
      );
    });
      ...
      <div className="articles">
        <div className="articles-wrapper">
          ...
          <div className="shop-right-articles">
            <div className="new-articles-cart">{cart /*const cart*/}</div>
          </div>
        </div>
